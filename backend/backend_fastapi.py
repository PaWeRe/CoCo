...
async def chat_completions(request: ChatCompletionRequest):
    ...
    cursor_rewrite = (
        "You are helping a colleague rewrite a piece of code"
        in request.messages[0].content
    )
    cursor_chat = (
        "You are happy to help answer any questions that the user has"
        in request.messages[0].content
    )
    ...
    # Collaboration branch for chat with human review
        ...
        if cursor_rewrite:
            print(
                "WARNING: Cursor Re-write doesn't have enough timeout for collaboratio. Try @coco_collab."
            )
        ...
        if resp is None:
            raise HTTPException(
                status_code=408, detail="Timed out waiting for human review"
            )
    ...
    # Baseline: no human review intervention
        ...
        print(
            f"***Baseline, cursor_rewrite: {cursor_rewrite}, cursor_chat: {cursor_chat}, "
